...
public final class ChatTextStyle {
    private static final int STOCK_PITCH = ChatGeometry.NORMAL_LINE_PITCH;
    ...
    private static final int SMALL_FONT_RISE = 4; // Plain 11 sits this many px higher than Plain 12 in a cell of the same pitch
    ...
    void sync() {
        ...
        if ((fresh || landed) && isActive()) restyle(font(), pitch(), icons(), fresh);
    }

    // A setting changed, or sizes were re-applied over a rebuild
    void reapply() {
        restyle(font(), pitch(), icons(), noteRebuild());
    }
    ...
    private boolean isActive() {
        return font() != STOCK_FONT || pitch() != STOCK_PITCH || icons() != ChatResizerConfig.ChatIcons.NORMAL;
    }

    // Spacing and icons only ride the small font; Normal keeps the game's stock pitch and its own icons
    private boolean small() {
        return config.useSmallFont();
    }

    private int font() {
        return small() ? SMALL_FONT : STOCK_FONT;
    }

    // Clamped rather than trusted: the range is a config-panel hint, and a zero would divide by zero below
    private int pitch() {
        return small() ? Math.min(STOCK_PITCH, Math.max(1, config.chatLineHeight())) : STOCK_PITCH;
    }

    private ChatResizerConfig.ChatIcons icons() {
        return small() ? config.chatIcons() : ChatResizerConfig.ChatIcons.NORMAL;
    }
    ...
    private void restyle(int font, int pitch, ChatResizerConfig.ChatIcons icons, boolean fresh) {
        ...
        // Plain 11 draws its line higher in the cell than Plain 12, more so the taller the pitch. Drop only the
        // text down onto the stock bottom; the row skeleton (which the scroll pin reads) stays put so it isn't undone
        int shift = font == SMALL_FONT ? pitch - STOCK_PITCH + SMALL_FONT_RISE : 0;
        ...
        for (Widget row : rows) {
            ...
            for (int i = slot; i < first; i++) {
                ...
                int top = i == slot + PART_RANK ? Math.max(0, y - lift + shift) : y + shift;
                // The body follows the new row height; the rank icon must not, or scaling the GRAPHIC to the pitch
                // stretches/squishes it (and fixed layout never rebuilds to reset it), so exclude it from the follow
                boolean followsRow = i != slot + PART_RANK && part.getHeight() == was;
                place(part, top, followsRow ? height : part.getHeight());
            }
            place(row, y, height);
            y += height;
        }
